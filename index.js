...
const ADMIN =
  ...
  const CONTRACT_TITLE = 'Key/Value store with admin';

const db = await Database.load(STORAGE, DB_PUBKEY);
console.log("DB loaded", db.pubkey.toString("hex"));
...
const contractCode = await db._readContractCode();
console.log("Expected Contract:", contractCode.indexOf(CONTRACT_TITLE) !== -1 ? `Yes, found "${CONTRACT_TITLE}".` : 'Nope... better check the contract code.');

const tx0 = await db.call("getAdmin");
console.log(
  tx0.response && tx0.response.value.pubkey === ADMIN
    ? "Admin OK"
    : "Unexpected Admin!"
); // => ADMIN
